...
| 内存     | 23 GiB                                   |
| 操作系统 | WSL2 / Linux 6.6.87                      |
| 拓扑     | 本机 3 节点（127.0.0.1:7001/7002/7003）  |
| 向量维度 | 128 维（`f32`，IP 距离）                 |
| 预热请求 | 500（search 模式 1000）                  |
| 统计请求 | 5000                                     |
| 代码版本 | commit `8d03eee`（Phase 1~3 全部修复后） |

---

### 写入吞吐（upsert）纵向扩展

| 并发线程 | 吞吐          | P50     | P99     | P999    | 错误率 |
| -------- | ------------- | ------- | ------- | ------- | ------ |
| 1 线程   | 163.3 ops/sec | 6.3 ms  | 7.8 ms  | 10.3 ms | 0%     |
| 4 线程   | 257.6 ops/sec | 11.8 ms | 61.9 ms | 77.2 ms | 0%     |
| 8 线程   | 347.1 ops/sec | 22.4 ms | 31.6 ms | 35.0 ms | 0%     |

### 读取吞吐（search，租约读）纵向扩展

| 并发线程 | 吞吐           | P50    | P99    | P999   | 错误率 |
| -------- | -------------- | ------ | ------ | ------ | ------ |
| 4 线程   | 2082.3 ops/sec | 1.8 ms | 2.3 ms | 2.5 ms | 0%     |
| 8 线程   | 2776.4 ops/sec | 2.6 ms | 3.3 ms | 3.7 ms | 0%     |

### 混合模式（4 线程）

| 模式                                         | 吞吐          | P50    | P99      | P999     | 错误率 |
| -------------------------------------------- | ------------- | ------ | -------- | -------- | ------ |
| mixed（70% upsert + 30% search）             | 233.6 ops/sec | 6.4 ms | 501.7 ms | 502.1 ms | 0.24%  |
| full（50% upsert + 20% delete + 30% search） | 284.0 ops/sec | 6.9 ms | 501.7 ms | 502.3 ms | 0.14%  |

---

### 结果分析

- **写入线性扩展**：upsert 吞吐从 1→4→8 线程呈 1×→1.58×→2.12× 线性增长，瓶颈在于 WAL `fdatasync`（Phase 3 P6 已优化为与 Pipeline RPC 并行）+ HNSW 图插入的写锁竞争。

- **读取高吞吐**：search 走租约读路径（本地 HNSW 索引，无网络往返），8 线程达 **2776 ops/sec**，P99 仅 3.3 ms，读写比约 **8:1**（8线程读 vs 1线程写），符合读多写少的 ANN 检索场景定位。

- **混合模式 P99 偏高**：mixed/full 的 P99 约 502 ms，对应 `HandleClientWrite` 的 `future.wait_for` 超时（写路径 Apply 等待时间）。在 4 线程并发写下，写入竞争导致 Apply 延迟增大；失败率 < 0.25%，属正常的超时重试范围，写入最终一致性仍为 100%。

- **全程零选举**：压测期间无非预期 Leader 切换，租约持续有效，体现了 Phase 1 P3（并行心跳）修复后租约稳定性的提升。

- **WSL2 说明**：WSL2 的 `fdatasync` 性能远低于裸机 Linux（WSL2 文件系统为 ext4 挂载于 Windows NTFS 之上），生产环境裸机 NVMe 下 upsert 吞吐预计可提升 3~5 倍。
